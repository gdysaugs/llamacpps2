...
class SOVITSWav2LipLlamaGradioApp:
    def __init__(self):
        ...
        # ポータブル環境対応の一時ディレクトリ (Docker対応)
        if os.path.exists("/app"):
            # Docker環境
            self.temp_dir = Path("/app/temp/gradio_sovits_wav2lip_llama")
        else:
            # ローカル環境
            ...
            if use_ai_conversation and self.llama_integration:
                ...
                try:
                    ...
                    if llama_result and llama_result.get("success"):
                        ...
                        log_messages.append("⚠️ AI会話失敗、元テキストを使用")

                except Exception as e:
                    log_messages.append(f"⚠️ AI会話エラー: {str(e)}")
    ...
    def cleanup_existing_processes(self):
        ...
        try:
            ...
            # 一時ファイルクリーンアップ (Docker対応)
            try:
                temp_dirs = [
                    "/tmp/sovits_wav2lip_integration",
                    "/tmp/gradio_sovits_wav2lip_llama",
                    "/app/temp/sovits_wav2lip_integration" if os.path.exists("/app") else "/tmp/sovits_wav2lip_integration",
...
def main():
    ...
    # ポートを8080に設定（Docker統合版として）
    # Docker環境対応のallowed_paths
    allowed_paths = []
    if os.path.exists("/app/output"):
        allowed_paths.append("/app/output")
    if os.path.exists("/app/temp"):
        allowed_paths.append("/app/temp")

    # ローカル環境のパスも追加
    allowed_paths.extend([str(APP_ROOT / "output"), str(APP_ROOT / "temp")])
